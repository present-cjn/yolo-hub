# app_demo/multi_thread_rtsp_demo/main.py
import cv2
import sys
import os
...
try:
    ...
    from core_lib.img_reader.rtsp_reader import RTSPReader  # <--- 使用RTSPReader
...
WINDOW_NAME = "YoloHub Multi-Thread RTSP Demo"
QUEUE_SIZE = 1  # 缓冲区大小，1表示只保留最新帧
...
try:
    ...
    # 1. 先创建原始的reader
    reader = RTSPReader(rtsp_url=RTSP_URL)
...
try:
    ...
    # 5. 清理资源
    print("正在停止读取线程并关闭窗口...")
    threaded_reader.stop()  # 停止线程 (会自动调用 reader.release())
